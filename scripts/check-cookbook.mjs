...
for (const locale of locales) {
  const root = join(contentRoot, locale, "cookbook");
...
function checkNavigation() {
  ...
  const cookbook = config.indexOf('label: "Cookbook"');
  const bestPractices = config.indexOf('label: "Best Practices"');
  if (!(installation < cookbook && cookbook < bestPractices)) {
    errors.push("astro.config.mjs: Cookbook must follow Installation and precede Best Practices");
  }
  ...
  if (!config.includes('directory: "cookbook"')) {
    errors.push("astro.config.mjs: Cookbook navigation is not autogenerated");
  }
}
